...
router.post('/', function (req, res, next) {
    ...
    if (req.body.action === "run_sqlite_test") {
        ...
        for (i = 1; i <= insert_nb; i++) {
            sqlite_test.insert(i + '@a.a', 'name', 'surname', '07/07/01', 'homme', 175, 60, 'blablabla')
        }
        ...
        for (i = 1; i <= insert_nb; i++) {
            mysql_test.insert(i + '@a.a', 'name', 'surname', '07/07/01', 'homme', 175, 60, 'blablabla')
        }
        dateInsert = new Date() - dateInsert;
        ...
        mongodb_db.connect(async (err) => {
            ...
            dateInsert = new Date();
            for (i = 1; i <= insert_nb; i++) {
                db.collection('test_rows').insertOne({'id': i + '@a.a', 'firstname': 'name', 'lastname': 'surname', 'birthday': '07/07/01', 'gender': 'homme', 'height': 175, 'weight': 60, 'pwd': bcrypt.hashSync('blablabla', 10)})
            }
            dateInsert = new Date() - dateInsert;
        ...
        dateInsert = new Date();
        for (i = 1; i <= insert_nb; i++) {
            redis_db.hset([i + '@a.a', 'test_rows', 'name', 'surname', '07/07/01', 'homme', 175, 60, 'blablabla'])
        }
        dateInsert = new Date() - dateInsert;
